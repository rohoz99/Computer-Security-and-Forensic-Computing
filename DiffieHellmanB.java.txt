...
public class DiffieHellmanB{
    ...
    public static void main(String[] args){
        ...
        long MalorysPrivateKey = generatePrivateKey(p);
        System.out.println("Malory's private key: " +MalorysPrivateKey);
        BigInteger MalorysGenerated = generatorMethod(a, p, MalorysPrivateKey);
        System.out.println("Public Key Generated by Malory: "+MalorysGenerated);

        BigInteger kAlice = generatorMethod(MalorysGenerated.longValue(), p, AlicesPrivateKey);
        BigInteger kBob = generatorMethod(MalorysGenerated.longValue(), p, BobsPrivateKey);

        System.out.println("K generated by Alice using Malory's public key: " + kAlice);
        System.out.println("K generated by Bob using Malory's public key: " + kBob);

        System.out.println("Attempting to calculate Alice's private key...");
        long alicesPrivateKey = crackPrivateKey(p, a, AlicesGenerated);
        System.out.println("Alice's private key is: "+alicesPrivateKey);
        System.out.println("Attempting to calculate Bob's private key...");
        long bobsPrivateKey = crackPrivateKey(p, a, BobsGenerated);
        System.out.println("Bob's private key is: "+bobsPrivateKey);
    }
    ...
    // A method to find a private key given p, a and the public key of the user
    public static long crackPrivateKey(long p, long a, BigInteger publicKey){
        long privateKey = 0;
        //brute force method to calculate
        for(int i=1; i<(p-1);i++){
            BigInteger generated = generatorMethod(a, p, i);
            if(generated.compareTo(publicKey) == 0){
               privateKey = generated.longValue();
            }
        }
        return privateKey;
    }
